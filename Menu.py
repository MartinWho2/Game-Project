...
class Menu:
    def __init__(self,bg):
        ...
        self.font_2 = pygame.font.SysFont('comicsans',int(self.bg.w/10)) #utilité à vérifier
        self.color_0 = (0,255,0)
        self.color_1 = (255,255,255)
        self.color_2 = (255,255,255) #utilité à vérifier
        self.selected = 0
        self.clicked = 0
    ...
    def menu(self):
        if self.event_happens:
            self.resume = self.font_0.render("Continue",True,self.color_0) #utilité à vérifier
            self.save = self.font_1.render("Save / Load", True, self.color_1) #utilité à vérifier
            self.quit = self.font_2.render("Quit", True, self.color_2) #utilité à vérifier

            self.resumeRect = self.resume.get_rect() #utilité à vérifier
            self.saveRect = self.save.get_rect() #utilité à vérifier
            self.quitRect = self.quit.get_rect() #utilité à vérifier
            self.rects = [self.resumeRect, self.saveRect, self.quitRect] #utilité à vérifier

            self.resumeRect.center = (int(self.bg.w / 2), int(self.bg.h * 2 / 7)) #utilité à vérifier
            self.saveRect.center = (int(self.bg.w / 2), int(self.bg.h / 2)) #utilité à vérifier
            self.quitRect.center = (int(self.bg.w / 2), int(self.bg.h * 5 / 7)) #utilité à vérifier
